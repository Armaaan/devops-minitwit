...
def check_sim_auth(authorization: str | None) -> JSONResponse | None:
...
async def get_latest():
    try:
        with open(LATEST_FILE) as fp:  # noqa: ASYNC230
...
async def follow_user(request: Request, username: str):
    ...
    db.commit()
    add_flash(session_data, f'You are now following "{username}"')
    db.close()
...
async def unfollow_user(request: Request, username: str):
    ...
    db.commit()
    add_flash(session_data, f'You are no longer following "{username}"')
    db.close()
    response = RedirectResponse(url=f'/{username}', status_code=302)
    set_session(response, session_data)
    return response
